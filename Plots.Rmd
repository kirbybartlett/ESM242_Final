...
merged_withdrawls <- left_join(SGMA_2040, Open_Access, by =c('t')) %>% 
   tidyr::gather(key = 'Withdrawl_Type', value = 'Withdrawl', SGMA_2040, Open_Withdrawl)
...
SGMA_years_plot <-
  ggplot() +
  geom_line(data = SGMA_2040, aes(t, SGMA_2040), size = 1) +
  geom_line(data = SGMA_2060, aes(t, SGMA_2060), size = 1) +
  geom_line(data = SGMA_2080, aes(t, SGMA_2080), size = 1) +
  geom_line(data = SGMA_2100, aes(t, SGMA_2100), size = 1) +
  scale_color_manual(values=colors) +
  theme_minimal_hgrid() +
  scale_x_continuous(expand = c(0,0)) +
  labs(x = "Year", y = "Withdrawls in Acre Feet") +
